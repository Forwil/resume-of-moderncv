...
\cventry{2016.12--至今}{研究员}{SenseTime 商汤科技}{研究中心}{智能视频（mentor:闫俊杰）}
{
带领5人小团队，负责智能视频组安防相关所有算法落地和升级。
\newline
1、搭建SenseVideo-GPU-SDK高性能视频结构化分析系统，支持行人车辆非机动车检测跟踪及属性提取。算法库从零开始搭建，负责SDK算法封装，流程设计以及性能优化。
\newline
2、作为监控算法嵌入式化的主要负责人，推动包括TX1/TX2（人脸跟踪识别服务器阵列）、海思3519（前端人脸抓拍相机)、Movidius芯片（自研神经网络加速库的前端人脸识别芯片）等前端产品落地。
\newline
3、负责支撑智能视频分析的GPU底层核心框架迭代和优化，包含TensorRT定点化加速，multi-steam-caffe，Net-rt通用神经网络处理框架。
}

\cventry{2016.3--2016.12}{见习研究员}{SenseTime 商汤科技}{研究中心}{检测跟踪组（mentor:闫俊杰）}
...
\newline
2、使用行人检测和ReID特征优化了多目标跟踪系统，在MOT16榜单上取得包括MOTA指标（68.2和66.1）在内的多项第一。
}
...
\newline
2、在总决赛中：负责集群软硬件平台搭建、功耗控制、HPL、HPCG调优、WRF-CHEM 应用优化和集群运行策略调度，最终队伍以全球第五名获得一等奖。} 
\cventry{2014.7--2014.12}{研发实习生}{Microsoft ARD 微软亚太研发集团，CEC - IoT Group}{北京}{}
{先后参与三个项目：
\newline
1、在智能插座项目中，为 STM32F 上的.Net Micro Framework 固件添加高级 ADC 操作；
\newline
2、在基于低功耗蓝牙的室内定位项目中，设计并实现在 51MCU 上的 RS-485 总线多对一通信协议；
\newline
3、在自动化测试项目中，提取测试程序调用外部库的依赖关系，存入数据库并对外提供 WCF 接口。} 
...
\cvitem{2016}{专利：一种基于卷积神经网络特征的多目标在线跟踪算法（受理）}
\cvitem{2016}{专利：基于目标特征点和产生式循环网络的多目标跟踪方法（受理）}
