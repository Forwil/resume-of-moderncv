...
{本科综合排名7/228，推免生保送入学。学位课程平均分：90.0/100，排名：6/241。研究方向：形式化验证编译器、Java虚拟机、自然语言处理。}
...
{维护动态人脸检测跟踪系统SenseFace-SDK，把系统移植到NVIDIA Tegra X1 嵌入式平台上，优化后满足实时性要求，于2016北京安博会展出。完成CPU版本的并行化方案设计、实现和优化，使系统可以实时处理1080P视频流。重构代码使得其满足公司自有的SDK规范并对外发布（sdkvideo），提出一种基于跟踪的检测框再调整策略，可以大大降低检测次数，提升系统速度。把sdkvideo移植到宇视摄像头芯片（armv7）中并优化检测跟踪速度2hz至16h。协助实习生使用movidius芯片进行深度学习算法加速，成功完成caffe model到mvtensor的自动转换工具链，并移植了人脸识别模型。实现并维护视频结构化系统SenseObject-SDK。在MOT16（Multiple Object Tracking）上，使用行人检测和re-id特征优化了Tracking算法，取得包括MOTA（68.2和66.1）在内的多项指标世界第一，以第一作者身份发表一篇ECCV workshop paper。}  
\cventry{2015.9--2015.11}{项目}{多功能WiFi路由器}{课程设计}{https://github.com/Forwil/embedded-socks}
...
{在初赛中：负责4台浪潮服务器组成超算小集群的软硬件搭建和维护，对 HPCC 的多个测试子项目（包括 Linpack、FFT、DGEMM）进行深入分析和编译优化，撰写英文proposal，队伍以初赛大陆第一，世界第二进入全球总决赛。在总决赛中：负责集群软硬件平台搭建、功耗控制、HPL、HPCG调优、WRF-CHEM 应用优化和集群运行策略调度，最终队伍以全球第五名获得一等奖。} 
...
\cventry{2013.9--2013.12}{项目}{“向小葵”点评网}{课程设计}{https://github.com/Forwil/xxk}
...
{负责批改作业、讲授习题课}
\cventry{2016.9—至今}{助教}{北航计算机学院编译原理}{本科课}{}
{负责批改作业、小测验、实验课习题课讲解}
\cventry{2016.9—至今}{助教}{北航计算机学院形式语言与自动机}{研究生课}{}
{负责批改作业，习题课讲解}
...
\cvitem{2016}{\textbf{Fengwei Yu}, Wenbo Li, Quanquan Li, Yu Liu, Xiaohua Shi, and Junjie Yan. POI: Multiple object
tracking with high performance detection and appearance feature. In Proceedings of the European Conference on Computer Vision （\textbf{ECCV}） 2016 Workshop (BMTT). \textbf{EI}.}
\cvitem{2016}{专利：一种基于卷积神经网络特征的多目标在线跟踪算法（在投）}
\end{document}
